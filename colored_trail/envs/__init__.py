...
class ColoredTrailEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, size=5, agent_num=4):
        ''' Parameters:
            render_mode: str
            size: int, board size
        ...
        self.chip_list = [self.ini_chip(agent_num, 4) for i in range(agent_num)]
        ...
        """ We have 4 moving actions, corresponding to "right", "up", "left", "down", "negotiation"
            change to 4+4*4 actions, add negotiation+direction+chip action
        ...
        """
        # TODO: Change action space to color to color
